...
@section('title', 'أقسام المقالات - لوحة التحكم')
@section('page-title', 'أقسام المقالات')
@section('page-description', 'إدارة وتنظيم أقسام المقالات على الموقع')
...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div class="flex items-center space-x-4 space-x-reverse">
            ...
            <div>
                ...
                <p class="text-gray-600">{{ $sections->total() ?? 0 }} قسم في النظام</p>
            </div>
        </div>
        
        <div class="flex items-center space-x-3 space-x-reverse">
            <a href="{{ route('admin.blog-sections.create') }}" 
    ...
    <div class="card-gradient rounded-2xl p-6 shadow-xl animate-fade-in-up">
        <form id="filtersForm" action="{{ route('admin.blog-sections.index') }}" method="GET">
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <div>
                    <select name="sort" 
                        ...
                        <option value="articles_count" {{ request('sort') == 'articles_count' ? 'selected' : '' }}>عدد المقالات</option>
                    </select>
                </div>
            </div>
        </form>
            ...
            <table class="w-full">
                <thead class="bg-gradient-to-r from-seeker-primary to-provider-primary text-white">
                    <tr>
                        ...
                        <th class="text-right p-4 font-semibold">الوصف</th>
                        <th class="text-center p-4 font-semibold">عدد المقالات</th>
                ...
                <tbody class="divide-y divide-gray-50">
                    @forelse($sections as $section)
                        <tr class="table-hover">
                            <td class="p-4">
                                <img src="{{ $section->thumbnail_url }}" class="w-12 h-12 rounded-lg object-cover">
                            </td>
                            <td class="p-4">
                                <div>
                                    <h3 class="font-semibold text-gray-800">{{ $section->title }}</h3>
                                    <p class="text-sm text-gray-500">{{ $section->slug }}</p>
                                </div>
                            </td>
                            <td class="p-4">
                                <p class="text-sm text-gray-600 max-w-xs line-clamp-2">
                                    {{ Str::limit($section->description, 25) }}
                                </p>
                            </td>
                            ...
                            <td class="p-4 text-sm text-gray-600">{{ $section->created_at->format('Y/m/d') }}</td>
                            <td class="p-4">
                                <div class="flex items-center justify-center space-x-2 space-x-reverse">
                                    <!-- View -->
                                    <div class="relative group">
                                        <a href="{{ route('admin.blog-sections.show', $section) }}" 
                                    ...
                                    <div class="relative group">
                                        <button type="button" 
                                                onclick="copySectionLink('{{ route('sections.show', $section->slug) }}')" 
                                    ...
                                    <div class="relative group">
                                        <button type="button" 
                                                onclick="confirmDelete('{{ route('admin.blog-sections.destroy', $section) }}')" 
...
function confirmDelete(url) {
    Swal.fire({
        title: 'هل أنت متأكد؟',
        text: 'لن تتمكن من التراجع عن هذا الإجراء وسيتم حذف جميع المقالات المرتبطة بهذ القسم',
        icon: 'warning',
